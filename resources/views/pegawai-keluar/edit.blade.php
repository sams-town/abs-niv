...
<div class="row">
    <div class="col-12 pk-form-wrapper">
        <div class="pk-form-header">
            <h4><i class="fa fa-user-edit me-2"></i>Edit Data Pegawai Keluar</h4>
            <p>Perbarui informasi keberhentian pegawai</p>
        </div>
        <div class="pk-form-body">
            <form method="post" action="{{ url('/exit/update/'.$pegawai_keluar->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="pk-select-wrapper">
                        <select class="pk-input @error('user_id') is-invalid @enderror" id="user_id" name="user_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $pegawai_keluar->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="pk-form-group">
                    ...
                    <input type="date" class="pk-input @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal"
                ...
                <div class="pk-form-group">
                    <div class="jenis-cards">
                        <div class="jenis-card-option">
                            <input type="radio" id="jenis_phk" name="jenis" value="PHK"
                        ...
                        <div class="jenis-card-option">
                            <input type="radio" id="jenis_mundur" name="jenis" value="Mengundurkan Diri"
                        ...
                        <div class="jenis-card-option">
                            <input type="radio" id="jenis_meninggal" name="jenis" value="Meninggal Dunia"
                        ...
                        <div class="jenis-card-option">
                            <input type="radio" id="jenis_pensiun" name="jenis" value="Pensiun"
                ...
                <div class="pk-form-group">
                    ...
                    <textarea name="alasan" id="alasan" class="pk-input @error('alasan') is-invalid @enderror" rows="4"
                        placeholder="Jelaskan alasan keberhentian pegawai...">{{ old('alasan', $pegawai_keluar->alasan) }}</textarea>
                    @error('alasan')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="pk-form-group">
                    ...
                    @if ($pegawai_keluar->pegawai_keluar_file_path)
                        <div class="mb-2">
                            <a href="{{ url('/storage/'.$pegawai_keluar->pegawai_keluar_file_path) }}" class="existing-file-badge" target="_blank">
                                <i class="fa fa-file-download"></i>
                                File saat ini: {{ $pegawai_keluar->pegawai_keluar_file_name }}
                            </a>
                        </div>
                    @endif
                    <div class="pk-file-area" id="fileArea">
                        <i class="fa fa-cloud-upload-alt fa-2x mb-2" style="color:#94a3b8;"></i>
                        <p style="color:#64748b; margin: 0; font-size:0.88rem;" id="fileLabel">
                            {{ $pegawai_keluar->pegawai_keluar_file_path ? 'Ganti file (opsional)' : 'Klik atau seret file ke sini' }}
                        </p>
