...
@section('isi')
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                <form method="post" class="p-4" action="{{ url('/rapat/store') }}">
                    @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control clockpicker @error('jam_mulai') is-invalid @enderror" id="jam_mulai" name="jam_mulai" autofocus value="{{ old('jam_mulai') }}">
                            @error('jam_mulai')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control clockpicker @error('jam_selesai') is-invalid @enderror" id="jam_selesai" name="jam_selesai" autofocus value="{{ old('jam_selesai') }}">
                            @error('jam_selesai')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control selectpicker @error('user_id') is-invalid @enderror" id="user_id" name="user_id[]" multiple data-actions-box="true" data-live-search="true" data-selected-text-format="count > 3" title="Pilih Peserta...">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" data-tipe="{{ $user->tipe_user == 'dosen' ? 'dosen' : 'karyawan' }}" {{ (is_array(old('user_id')) && in_array($user->id, old('user_id'))) ? 'selected' : '' }}>
