...
<div class="container-fluid py-4" style="background: #f8fafc; min-height: 100vh;">
    <form method="post" action="{{ url('/role/update/' . $role->id) }}" id="roleForm">
        @method('PUT')
        @csrf
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h3 style="font-weight: 800; color: #0f172a; margin: 0;">Edit Role</h3>
                    <p class="text-muted mb-0" style="font-size: 13px;">Atur hak akses jabatan</p>
                </div>
            </div>
            ...
            <div class="col-lg-3">
                <div class="card shadow-sm border-0" style="border-radius: 20px; background: #ffffff; padding: 24px;">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $role->name) }}" required style="border-radius: 12px; padding: 12px 16px; background-color: #f8fafc; border: 1px solid #cbd5e1; font-weight: 600; color: #0f172a;">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="guard_name" name="guard_name" value="{{ old('guard_name', $role->guard_name) }}" readonly style="border-radius: 12px; padding: 12px 16px; background-color: #f1f5f9; border: 1px solid #cbd5e1; font-weight: 600; color: #64748b;">
                    </div>
                </div>
            </div>
                ...
                <div class="card shadow-sm border-0" style="border-radius: 20px; background: #ffffff; padding: 24px;">
                    ...
                    <div class="row g-3">
                        @foreach($permissions as $category => $items)
                            @php
                                $total = $items->count();
                                $checkedCount = $items->filter(fn($item) => in_array($item->name, $rolePermissions))->count();
                                $isAllChecked = ($total === $checkedCount);
                                ...
                                <div class="card p-3 h-100" style="border: 1px solid #f1f5f9; border-radius: 16px; background-color: #ffffff; transition: border-color 0.2s;">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center pb-2 mb-3" style="border-bottom: 1px solid #f1f5f9;">
                                        <div class="form-check">
                                            <input class="form-check-input select-all-category" type="checkbox" id="cat_{{ $category }}" data-target-class="chk_{{ $category }}" {{ $isAllChecked ? 'checked' : '' }} {{ $isIndeterminate ? 'data-indeterminate=true' : '' }}>
                                    ...
                                    <div class="d-flex flex-column gap-2">
                                        @foreach($items as $permission)
                                            ...
                                            @endphp
                                            <div class="form-check">
                                                <input class="form-check-input permission-checkbox chk_{{ $category }}" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="perm_{{ $permission->id }}" {{ $isChecked ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Handle initial indeterminate state
        document.querySelectorAll('.select-all-category').forEach(function (selectAll) {
            if (selectAll.getAttribute('data-indeterminate') === 'true') {
